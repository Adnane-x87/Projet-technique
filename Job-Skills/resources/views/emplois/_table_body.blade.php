@forelse($emplois as $emploi)
    <tr class="hover:bg-gray-50 dark:hover:bg-zinc-900 transition-colors">
        <td class="px-6 py-4 whitespace-nowrap">
            <div class="flex items-center">
                @if ($emploi->image)
                    <img src="{{ asset('storage/' . $emploi->image) }}" class="h-10 w-10 rounded-lg object-cover mr-3 border border-gray-100 dark:border-zinc-800 shadow-sm">
                @else
                    <div class="h-10 w-10 rounded-lg bg-gray-100 dark:bg-zinc-800 flex items-center justify-center mr-3 border border-gray-100 dark:border-zinc-800">
                ...
                <div>
                    ...
                    <div class="text-xs text-gray-500 dark:text-zinc-400">{{ $emploi->company }}</div>
                </div>
            </div>
        </td>
        <td class="px-6 py-4">
        ...
        <td class="px-6 py-4 whitespace-nowrap">
            <div class="text-sm text-gray-500 dark:text-zinc-400">{{ $emploi->created_at->diffForHumans() }}</div>
        </td>
...
@empty
    <tr>
        <td colspan="4" class="px-6 py-12 text-center text-gray-500 dark:text-zinc-400">
            <div class="flex flex-col items-center">
                <i data-lucide="folder-open" class="h-10 w-10 text-gray-300 mb-3 dark:text-zinc-700"></i>
